    ...
    while True:
        try:
            ...
            if 1 <= row <= size and 1 <= col <= size:
                ...
                if (row + length <= size and all(board[row + i][col] == 'O' for i in range(length))) or \
                   (col + length <= size and all(board[row][col + i] == 'O' for i in range(length))):
                    break
                else:
    ...
    while True:
        ...
        if (row + length <= size and all(board[row + i][col] == 'O' for i in range(length))) or \
           (col + length <= size and all(board[row][col + i] == 'O' for i in range(length))):
            ...
            # Check if the chosen position is far from other ships
            far_from_other_ships = True
            for ship_row, ship_col, ship_length in ships:
                if abs(row - ship_row) < 2 * length and abs(col - ship_col) < 2 * length:
...
def battleship_game():
    ...
    if place_option == 'yes':
        ...
        for i in range(num_ships):
            ...
            user_row, user_col = place_ship_manually(user_board, size, length, ship_name)
            user_ships.append((user_row, user_col, length))  # Only store the first letter of the ship name
        ...
        for i in range(num_ships):
            ...
            user_row, user_col = place_ship_randomly(user_board, size, length, user_ships)
            user_ships.append((user_row, user_col, length))  # Only store the first letter of the ship name
            for i in range(length):
                if i == 0:
                    user_board[user_row][user_col] = ship_name[0]
    ...
    for i in range(num_ships):
        ...
        computer_row, computer_col = place_ship_randomly(computer_board, size, length, computer_ships)
        computer_ships.append((computer_row, computer_col, length))  # Only store the first letter of the ship name
        for i in range(length):
            if i == 0:
    ...
    num_turns = size * size  # Maximum number of turns = number of cells on the grid
    for turn in range(num_turns):
        ...
        if (row_guess, col_guess) == (computer_row, computer_col):
        ...
        # Computer's turn
        computer_guess_row, computer_guess_col = place_ship_randomly(computer_board, size, 1, computer_ships)

        print("\nComputer's Guess:")
        print(f"Row: {computer_guess_row + 1}, Column: {computer_guess_col + 1}")

        if (computer_guess_row, computer_guess_col) == (user_row, user_col):
